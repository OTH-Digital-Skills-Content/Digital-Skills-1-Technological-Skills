...
* Sollte ihr Codespace die Befehle ```check``` und ```submit```nicht verstehen, haben Sie möglicherweise nicht lange genug gewartet und eine Eingabe im Terminal gemacht, bevor der codespace vollständig konfiguriert wurde. Richten Sie den Container wie folgt neu ein: "Shift + Cmd + P" (Mac) oder "Ctrl + Shift + P" (Windows). Geben Sie in das jetzt erschienene Eingabefeld den folgenden Text ein und bestätigen mit Enter: >Rebuild Container
