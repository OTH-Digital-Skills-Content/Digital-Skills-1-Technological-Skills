...
Im Spiel **Minecraft** benötigen Sie für eine sichere Nacht eine solide **Mauer** – genauer: eine quadratische Mauer aus Blöcken.  
Ihre Aufgabe ist es, diese **quadratische Mauer** in Python nachzubauen, wobei jedes Element der Mauer aus einem ```#```-Symbol oder einem `*`-Symbol besteht (stellen Sie sich jedes ```#``` und jeden `*` als einen Minecraft-Block vor), so ähnlich wie in der folgenden Abbildung:
...
Beispiel für eine Mauer der Größe `4`:
~~~
#*#*
*#*#
#*#*
...
3 Iteriere von 1 bis zur Höhe und
	Gebe in Iteration i, i # bzw. * Zeichen aus und anschließend eine neue Zeile
~~~

### User nach Input fragen

Beginnen Sie damit (egal, wie Ihr Pseudocode aussieht) den Code zu schreiben, der die Nutzer (wiederholt) dazu auffordert eine Höhe einzugeben. Öffnen Sie dazu die Datei ```blocks.py``` in Codespaces. 

Passen Sie ```blocks.py``` jetzt so an, dass das Programm die Benutzer immer wieder auffordert eine zulässige Zahl zwischen 1 und 8 einzugeben und speichern Sie dieses Ergebnis in einer Variable ab. Geben Sie dann die gespeicherte Höhe wie folgt aus:
...
Nachdem das Programm die Eingabe der Nutzer akzeptiert, geht es an die Ausgabe der Mauer. Eine Mauer aus gleichen Zeichen lässt sich leichter ausgeben:
...
* Schleifen mit `for` dienen in Python dazu Befehle öfter auszuführen. Zusätzlich können Sie aber auch bei jeder Wiederholung den Wert einer Zählvariablen ausgeben. Führt man den folgenden Code ...
  ...
  Jede Zeile steht für eine Wiederholung von `print(i)`. Die Ziffern `0`, `1` und `2` repräsentieren den Wert der Variable `i`, die bei jedem Durchgang um 1 erhöht wird. Sehen Sie sich die Notes zur Vorlesung genau an, und überlegen Sie, wie Sie diese Informationen nutzen können, um zu unterscheiden, ob sie ein `#` oder einen `*` ausgeben sollen. Starten Sie mit der gleichen Abfolge für jede Reihe (d.h. jede Reihe beginnt mit einem `#`. Überlegen Sie erst dann, wie Sie die Zeichen abwechseln können).

* Sie können `for`-Schleifen auch mit zwei Zählvariablen verschachteln.
