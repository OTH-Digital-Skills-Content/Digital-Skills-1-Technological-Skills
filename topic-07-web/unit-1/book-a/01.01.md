...
9. Integrieren Sie mindestens 1 Feature von Bootstrap in Ihre Website (**es werden nur die folgenden Komponenten von dem automatischen Test erfasst:** Ein Container (Klasse ```container```), Zeilen mit Spalten (Klassen ```row``` und ```col```), eine  Navigation (Klasse ```navbar```), ein Button (Klasse ```btn```), eine Card (Klasse ```card```) oder ein Popup mit Modal (Klasse ```modal```).
