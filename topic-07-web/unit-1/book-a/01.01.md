...
## Vorbereitung
1. Klicken Sie auf den folgenden [Link](https://classroom.github.com/a/0JtssDOL)
...
9. Integrieren Sie mindestens 1 Feature von Bootstrap in Ihre Website (**hierzu werden nur die folgenden Komponenten von dem automatische Test erfasst:** Ein Container (Klasse ```container```), Zeilen mit Spalten (Klassen ```row``` und ```col```), eine  Navigation (Klasse ```navbar```), ein Button (Klasse ```btn```), eine Card (Klasse ```card```) oder ein Popup mit Modal (Klasse ```modal```).
10. Integrieren Sie mindestens ein Feature von JavaScript im ```<head>``` Ihrer HTML-Datei, um die Interaktivität Ihrer Website zu erhöhen. Sie können JavaScript beispielsweise verwenden, um einen alert oder um Interaktivität zu Buttons, Dropdowns oder Formularen hinzuzufügen. Seien Sie kreativ! 
