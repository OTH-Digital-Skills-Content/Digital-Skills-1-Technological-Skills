...
2. Klicken Sie auf den folgenden [Link](https://classroom.github.com/a/mfvtNqxx)
...
Klicken Sie auf das Repository, für das Sie überprüfen wollen, ob die Abgabe funktioniert hat (z.B. ```OTH-Digital-Skills-Student-Submissions/lab-04-blocks-mheckneroutlook```). Auf der jetzt geöffneten Seite können Sie Ihren Code überprüfen (hier ```blocks.py```).
